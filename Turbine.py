...
class Turbine(Nature):
    ...
    def power(self):
        ...
        """
        return round((0.5 * self.xi * self.rho * self.area * self.w.v0**3) / 1000, 2)
    ...
    def direction(self, turbine):
        ...
        """
        if self.x == turbine.x and self.y < turbine.y:
            return pi/2
        if self.y == turbine.y and self.x < turbine.x:
            return float(0)
        if self.x > turbine.x or self.y > turbine.y:
            return -1       # code for detecting that turbine is not make interest for calculation
